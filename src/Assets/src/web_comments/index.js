app.controller('request', function($scope, $http, $compile, $window, $sce) {
    ...
    $scope.init = () => {
        ...
        $scope.sourse = all_source;
        $scope.post_list = all_post;
        $scope.page_show_title = 'Tất cả bình luận';

        $scope.getCounselling(filter_id);
    ...
    $scope.deleteCommentCofirmed = () => {
        $http.get(base_url + 'web_comments/ajax_delete_coment/' + $scope.delete_id).then(r => {
    ...
    $scope.sent_note = (value, index) => {
        if (!value.adv_note || value.adv_note == "") {
        ...
        var data = {
            ...
            bot_gender: value.bot_gender,
            public: 1,
            comment: value.adv_note
        }
        $http.post(base_url + 'web_comments/ajax_sent_counselling/' + value.id, JSON.stringify(data)).then(r => {
            if (r && r.data.status == 1) {
    ...
    $scope.open_reply_form = (event, item) => {
    ...
    $scope.set_collapse_status = (id, value, type) => {
        ...
        $http.post(base_url + 'web_comments/ajax_change_collapse_status', JSON.stringify(data)).then(r => {
            if (r && r.data.status == 1) {
    ...
    $scope.createFilter = (type, text, id, event) => {
        ...
        switch (type) {
            case 'page':
                ...
                $http.get(base_url + 'web_comments/ajax_get_post_list/' + id).then(r => {
                    if (r && r.data.status == 1) {
    ...
    $scope.open_news = (page_id, post_id) => {
        ...
        $http.get(base_url + 'web_comments/ajax_get_filter_name?filter=' + JSON.stringify($scope.filter)).then(r => {
            if (r && r.data.status == 1) {
    ...
    $scope.getCounselling = (filter_id = 0) => {
        if (filter_id != 0) {
            $scope.filter.filter_id = filter_id;
        } else {
            $scope.filter.filter_id = 0;
        }
        
        $scope.view_type = 'all';
        $http.get(base_url + 'web_comments/ajax_get_counselling?filter=' + JSON.stringify($scope.filter)).then(r => {
            if (r && r.data.status == 1) {
                $scope.counsellings = r.data.data;

                angular.forEach($scope.counsellings, function(value, key) {
                    value.bot_gender = '1';
                });
    ...
    $scope.changeFilterSource = () => {
        $scope.getCounselling();
    }

    $scope.getCounselling_2nd = () => {
        $http.get(base_url + 'web_comments/ajax_get_counselling_2nd?filter=' + JSON.stringify($scope.filter)).then(r => {
            if (r && r.data.status == 1) {
                $scope.counsellings = r.data.data;
    ...
    $scope.public_comment = (val, checked_status, type) => {
        ...
        $http.post(base_url + 'web_comments/ajax_change_public', JSON.stringify(data)).then(r => {
            if (r && r.data.status == 1) {
                toastr["success"]("Cập nhật thành công!");
                $('.comment-item').removeClass('show-actions');
