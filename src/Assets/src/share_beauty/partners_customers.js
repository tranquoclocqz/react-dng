app.controller('partners_customers', function ($scope, $sce, $http, $compile) {
    ...
    $scope.getAll = (reload = 0) => {
        ...
        $http.get(base_url + 'share_beauty/ajax_get_partners_customers?filter=' + JSON.stringify($scope.filter)).then(r => {
    ...
    $scope.inviterRoles = (customer) => {
        $http.get(base_url + 'share_beauty/ajax_get_role_inviters/' + customer.id).then(r => {
            if (r && r.data.status == 1) {
                $scope.customer_md_name = customer.name;
                $scope.customer_md = customer;
                $scope.inviters = r.data.data;
                $('#roles-moadal').modal('show');
            } else toastr["error"](r.data.messages);
        })
    }
    $scope.changePartnerRole = (item) => {
        var data = {
            'id': item.id,
            'customer_id': item.customer_id,
            'partner_id': item.partner_id,
            'customer_sb_type': item.customer_sb_type
        }
        $http.post(base_url + 'share_beauty/api_change_partner_role', data).then(r => {
            if (r && r.data.status == 1) {
                toastr["success"]("Đã thay đổi!");
                $scope.getAll();
                $scope.inviterRoles($scope.customer_md);
            } else {
