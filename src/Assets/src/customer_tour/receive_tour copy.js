...
angular.module('app', [])
    .controller('timeCtrl', function ($scope, $http, $compile, $window) {
        $scope.init = () => {
            $scope.invoices = {};
        ...
        $scope.removeSearch = () => {
            $('#modal_list_technician input').val('').trigger('input');
            $scope.list_technician_temp = $scope.list_technician;
        }
                ...
                if (result.value) {
                    ...
                    $http.post(url_api + 'ajax_set_tour_technician', {
                        ...
                        if (r.data.status == 1) {
                            ...
                            $('#modal_list_technician').modal('hide');
...
function showListTechnician(invoice_id) {
    ...
    $('#modal_list_technician input').val('').trigger('input');
    $('#modal_list_technician').modal('show');
}
...
// $(document).on('shown.bs.modal', '#modal_list_technician', function () {
//     $('#modal_list_technician input').focus();
// });
...
function createQrcode(invoice_id, customer_name) {
    console.log(invoice_id);
    let url = base_url + `customer_tour/invoice_info?invoice_id=${invoice_id}?user_id=656`;
    // url = base_url + 'dng_love?type=2%26receiver_id=' + invoice_id;
    $("#modal_qrcode .modal-body").html(`
            <div class="wrap-qrcode">
                <img src="https://chart.googleapis.com/chart?cht=qr&amp;chl=${url}&amp;chs=300x300&amp;chld=L|0">
            </div>
            <p class="mg-b-0"><b>${customer_name}</b></p>
        `);
