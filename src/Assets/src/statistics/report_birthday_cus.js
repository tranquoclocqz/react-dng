app.controller('report_birthday_cus', function ($scope, $http) {
    ...
    $scope.init = () => {
        ...
        $scope.filter.is_check_birthday = '-1';
        $scope.filter.date = "0";
    ...
    $scope.getRanks = () => {
        $http.get(base_url + '/statistics/ajax_get_ranks').then(r => {
            if (r && r.data.status == 1) {
                $scope.ranks = r.data.data;
    ...
    $scope.getCustomerBirth = () => {
        ...
        $http.get(base_url + '/statistics/ajax_get_customer_birthday?filter=' + JSON.stringify($scope.filter)).then(r => {
