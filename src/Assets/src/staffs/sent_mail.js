app.controller('sent_mail', function ($scope, $http) {
    ...
    $scope.confirm = (id) => {
        ...
        $http.post(base_url + '/staffs/ajax_confirm_mail/' + id, $scope.useravid).then(r => {
            if (r && r.data.status == 1) {
    ...
    $scope.showDetailMail = (id) => {
        ...
        $http.get(base_url + '/staffs/ajax_get_detail_mail/' + id).then(r => {
            ...
            if (r && r.data.status == 1) {
                ...
                $scope.ids = [];
                $scope.useravid = $scope.mailDetail.created_user_id;
    ...
    $scope.getWaitMailByUser = () => {
        ...
        $http.get(base_url + '/staffs/ajax_get_wait_mail_by_user/?filter=' + JSON.stringify($scope.filter)).then(r => {
            if (r && r.data.status == 1) {
    ...
    $scope.editMail = (item) => {
        ...
        $http.post(base_url + '/staffs/ajax_edit_mail/', item).then(r => {
            ...
            $scope.getWaitMailByUser($scope.currentGroupId);
