app.controller('dng_love_setting', function ($scope, $http, $compile) {
    ...
    $scope.sendHearts = (count = false) => {
        ...
        $http.post(base_url + 'admin_users/ajax_send_hearts', $scope.heartSend).then(r => {
    ...
    $scope.getHeartRank = () => {
        $http.get(base_url + 'admin_users/ajax_get_heart_rank/1').then(r => {
            if (r && r.data.status == 1) {
                $scope.rankingData = r.data.data;
                console.log($scope.rankingData.length);
            }
        })
    }
    ...
    $scope.getloveUnit = () => {
        ...
        $http.get(base_url + 'admin_users/ajax_get_love_unit?filter=' + JSON.stringify(data)).then(r => {
            if (r && r.data.status == 1) {
    ...
    $scope.setSelectedGift = () => {
        obj = _.find($scope.unit_types, function (obj) { return obj.id == $scope.heartSend.unit_id })
        $scope.selectedGiftQuantity = obj.slt;
    }
    $scope.select2 = () => {
        setTimeout(() => {
            $('.select2').select2({
    ...
    $scope.load_policy = () => {
        $http.get(base_url + 'admin_users/ajax_get_policy_txt').then(r => {
            if (r && r.data.status == 1) {
    ...
    $scope.updatePolicy = () => {
        ...
        $http.post(base_url + 'admin_users/ajax_update_policy_txt', $scope.policies).then(r => {
            if (r && r.data.status == 1) {
    ...
    $scope.getAll = () => {
        ...
        $http.get(base_url + 'admin_users/get_all_love_gift?filter=' + JSON.stringify($scope.filter)).then(r => {
