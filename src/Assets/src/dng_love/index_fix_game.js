...
app.controller('index', function($scope, $http, $sce) {
    ...
    $scope.get_employess = (type = 0, limit = 0) => {
        ...
        $http.get(base_url + 'dng_love/ajax_get_employess?filter=' + JSON.stringify($scope.employeeFilter)).then(r => {
    ...
    $scope.createTrans = () => {
        ...
        if (data.quantity <= 0) {
            toastr["error"]('Vui lòng kiểm tra số lượng!');
            return false;
        }
    ...
    $scope.buyRedEnvelopes = () => {
        var count = ($scope.envQuantity.s + '').replace(/,/g, "");
        $http.get(base_url + 'dng_love/send_buy_enve/' + count).then(r => {
            if (r && r.data.status == 1) {
                toastr.success('Đã quy đổi.');
                getNewYearHistoties();
                $scope.noturn = false;
            } else {
                toastr["error"](r.data.messages);
            }
        })
    }
    $scope.hideBuy = () => {
    ...
    $scope.changefirework_time = (t) => {
        $scope.firework_time = t;
    }
    $scope.firework = (id) => {
        if ($scope.balance_event.fireworks_turn == 0) {
            // toastr["warning"]('Đã hết lượt');
            $scope.envQuantity = {};
            $scope.noturn = true;
            return false;
        }
        ...
        $scope.bumbing = true;
        $('.lx' + id).addClass('pushReleaseTo');
        setTimeout(() => {
            $http.get(base_url + 'dng_love/fireworks?times=' + $scope.firework_time).then(r => {
                $scope.bumbing = false;
                $('.pushReleaseTo').removeClass('pushReleaseTo');
                if (r.data.status == 1) {
                    $('#tet-modal').modal('show');
                    $scope.firework_result = r.data.data;
                    getNewYearHistoties();
                } else {
                    $scope.nogift = true;
                    toastr["error"](r.data.messages);
    ...
    $scope.changeGameplay = (type) => {
        $scope.game_play = type;
        delete $scope.employeeFilter.text;
        $scope.get_employess(0, 15);
    }
    $scope.openStealMd = (type, data = {}) => {
        $scope.stealStatus = type;
        if (type == 'manh') {
            $scope.detailImgData = data;
            getImagePiece(data.id);
        }
    }

    function getImagePiece(image_id) {
        $http.get(base_url + 'dng_love/ajax_get_image_piece/' + image_id).then(r => {
            if (r.data.status == 1) {
                $scope.detailPiece = r.data.data;
            }
        })
    }
    $scope.pieceExchange = () => {
        if ($scope.detailPiece.length < $scope.detailImgData.total_piece) {
            toastr["warning"]('Chưa thu thập đủ mảnh');
            return false;
        }
        if ($scope.exchangeLoad) {
            toastr["warning"]('Đang thực hiện');
            return false;
        }
        $scope.exchangeLoad = true;
        setTimeout(() => {
            $http.get(base_url + 'dng_love/ajax_image_piece_exchange/' + $scope.detailPiece[0].image_id).then(r => {
                $scope.exchangeLoad = false;
                if (r.data.status == 1) {
                    $scope.detailPiece = r.data.data;
                    getGameBag();
                    getNewYearHistoties();
                    toastr["success"]('Đã đổi thành công');
                    toastr["success"]('+ ' + (r.data.old_rs) + ' BLX');
                }
            })
        }, 1500);
    }
    $scope.heartSteal = (item) => {
        if ($scope.bumbing) {
            toastr["warning"]('Đang thực hiện');
            return false;
        }
        $scope.attack_name = item.user_name;
        $scope.bumbing = true;
        $('.avt' + item.id).addClass('vibration');

        $http.get(base_url + 'dng_love/ajax_stealing/' + item.id).then(r => {
            $scope.bumbing = false;
            $('.vibration').removeClass('vibration');
            if (r.data.status == 1) {
                getNewYearHistoties();
                if ($scope.employee_search == 1) {
                    $scope.get_employess();
                } else {
                    $scope.get_employess_event();
                }
                $scope.stealStatus = 'steal';
                $scope.stealRs = r.data.data;
                if (r.data.data.piece) {
                    toastr["success"]('Nhận ' + r.data.data.piece.name + ' ' + r.data.data.piece.i_name);
                }
            } else toastr["error"](r.data.messages);
