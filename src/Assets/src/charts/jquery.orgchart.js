...
(function(factory) {
    ...
    OrgChart.prototype = {
        ...
        bottomEdgeClickHandler: function(event) {
            // debugger
            // var attr = $('.orgchart').attr('transform');
            if (!$('.orgchart').hasClass('l2r')) {
                var mtriX = 1,
                    mtriY = 1,
                    scaleX = 0,
                    scaleY = 0;
                $('.orgchart').css(`transform`, `matrix(${mtriX}, 0, 0, ${mtriY})`);
        ...
        hEdgeClickHandler: function(event) {
            if (!$('.orgchart').hasClass('l2r')) {
                var mtriX = 1,
                    ...
                    scaleX = 0,
                    scaleY = 0;
                var attr = $('.orgchart').css('transform');
                if (attr != 'none') {
                    attr = attr.substring(7, attr.length - 1);
                    var arr = attr.split(",");
                    mtriX = arr[0];
                    mtriY = arr[3];
                    if (mtriX[0] < 0.5 || mtriY[3] < 0.5) {
                        scaleY -= 50;
                    }
                }
                $('.orgchart').css('transform', `matrix(${mtriX}, 0, 0, ${mtriY})`);
