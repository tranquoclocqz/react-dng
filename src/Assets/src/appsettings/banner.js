app.controller('bnCtrl', function ($scope, $http) {
        ...
        $scope.init = () => {
            ...
            $scope.getAll();
            // $scope.getBannerActive();
            $scope.slide = [];
        }

        $scope.getAll = () => {
            $http.get(base_url + '/appsettings/ajax_get_banner?filter=' + JSON.stringify($scope.filter)).then(r => {
        ...
        $scope.getBannerActive = () => {
            $http.get(base_url + '/appsettings/ajax_get_banner_active').then(r => {
                $scope.slide = r.data.data;
            })
        }
        ...
        $scope.saveBanner = () => {
            ...
            if (check) {
                $('#bannerMD').modal('hide');
                $http({
                    url: base_url + '/appsettings/ajax_created_banner',
                    method: "POST",
                    data: data,
                    headers: {
        ...
        $scope.changeActive = (val) => {
            $http.post(base_url + '/appsettings/ajax_update_active', val).then(r => {
                if (r.data && r.data.status == 1) {
